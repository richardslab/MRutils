...
missing <- c("rs7718395", "rs3822868", "rs10887718", "rs8018720", "rs2909218", "rs2229742")

missing %in% significant_snps$rsid
missing %in% exposure_and_proxies$rsid
...
missing %in% merged_and_combined$rsid
...
ld_pairs <- get_LD_pairs(merged_and_combined, population = params$pop, token = params$LDLink_token)

ld_pairs %<>% mutate(variable = factor(variable, levels = unique(RS_number)),
                     RS_number = factor(RS_number, levels = unique(RS_number)))
...
missing %in% merged_and_combined$rsid
missing %in% pruned_combined_snps$rsid
...
print(glue_collapse( c( glue("Removed {length(removed_rsids)} snps due to high LD (removed): "),glue("{removed_rsids} "))))

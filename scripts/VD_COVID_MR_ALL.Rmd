...
write.table(proxies_in_outcome_with_missing_ids, glue("{params$results_dir}/{params$prefix}_proxy_rsIdsFromdbSnp.txt"), quote = F, sep = '\t', row.names = F)

```
found rsids for `r nrow(proxies_in_outcome_with_missing_ids)` SNPs
```{r merge with rest of outcome}
proxies_in_outcome_with_missing_ids <- read.table(glue("{params$results_dir}/{params$prefix}_proxy_rsIdsFromdbSnp.txt"), quote = F, sep = '\t')

...
data_proxies <- merge(proxies_in_outcome, proxies, by = c("rsid","CHR","POS"),suffixes = c(".out",".exp"))

chimeric <- c("(A/T)", "(T/A)", "(C/G)", "(G/C)")
proxies_in_covid <- merge(proxies, data_proxies) %>%
...
merged_with_proxies <- merge(significant_VD_snps, 
                              select(proxies_in_covid, -MAF), 
                              suffixes = c(".exp",".out"), 
                              by.x = c("rsid","CHR"), 
                              by.y=c("query_rsid","CHR")) %>% select(-MAF) %>%
  select(c( -"R2", -"Correlated_Alleles", -"Alleles", -"Locus", -"POS.out")) 

names(merged_with_proxies)[which(names(merged_with_proxies) == "rsid.out")] <- "proxy_rsid"
names(merged_with_proxies)[which(names(merged_with_proxies) == "POS.exp")] <- "POS"

merged_and_combined <- merged_with_proxies %>% 
rbind(mutate(merged_A2_and_VD, proxy_rsid = NA), stringsAsFactors = FALSE)

...
params:
  ...
  exposure_pvalue_threshold: 5e-5
...
significant_snps <- filter_and_write_exposure_data(data, location_prefix = params$results_dir, params$exposure_pvalue_threshold, params$MAF_rare_threshold)
...
write.table(withIds, glue("{params$results_dir}/rsIdsFromdbSnp.txt"), quote = F, sep = '\t', row.names = F)
```

```{r post-cache read variable}
withIds <- read.table(glue("{params$results_dir}/rsIdsFromdbSnp.txt"), sep = '\t', header = T)
...
print(glue("there are {nrow(outcome_with_rsids%>%subset(is.na(rsid)))} variants lacking rsid, and {nrow(outcome_with_rsids)} variants overall."))
...
proxies <- get_proxies(subset(merged_exp_and_out, is.na(MAF.out))$rsid, params$LDLink_token, population = params$pop, results_dir = params$results_dir, skip_api = TRUE)

print(glue("Found {nrow(proxies)} proxies for {length( unique(proxies$query_rsid))} variants, out of {length(which(is.na(merged_exp_and_out$MAF.out)))} requested"))
```

```{r use proxies}
#subset to proxies that are in the outcome first, since we need to find out which proxies are present in the outcome
proxies_in_outcome <- extract_snps_from_bgzip(params$outcome_file, snps = proxies) %>% map_outcome_data()

proxies %<>% subset(rsid %in% proxies_in_outcome$rsid)

print(glue("{nrow(proxies)} proxies were found in the outcome."))
...
print(glue("and {nrow(exposure_and_proxies)} proxies were merged with the exposure."))
...
fixed_proxies_with_exposure <- adply(proxies_with_exposure, 1, function(x) {
  replace_alleles(list(x$Alleles, x$EA, x$NEA), x$Correlated_Alleles) %>%
    {
  data.frame(Alleles = .[1],
    EA = .[2],
    NEA = .[3]
    )}
  })
...
pruned_combined_snps <- prune_snps(merged_and_combined, population = params$pop, token = params$LDLink_token)

print(glue("Total number of SNPs for use in 2-Sample MR (pruned): {nrow(pruned_combined_snps)}."))

write.table(pruned_combined_snps, file = glue("{params$results_dir}/SNPsFor2SMR.txt"), sep = '\t', quote = F, row.names = F)
```

```{r get MR results}
results <- get_2smr_results(glue("{params$results_dir}/SNPsFor2SMR.txt"))
```
